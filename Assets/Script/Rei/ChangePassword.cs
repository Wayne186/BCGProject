...
public class ChangePassword : MonoBehaviour {
	...
	void Start () {
		try {
			jsonString = File.ReadAllText(Application.persistentDataPath + "/User.json");
		} catch {
			jsonString = (Resources.Load("User") as TextAsset).ToString();
		}

		if (jsonString != null) {

			IList userInfo = (IList)Json.Deserialize (jsonString);
